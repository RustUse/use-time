...
1. Create or reuse a crates.io API token with publish access for `use-time`.
...
After the first manual crates.io release for `use-time` exists, the repository
...
- Purpose: confirms `use-time` already exists on crates.io, then runs
  `release-plz release`
...
```sh
cargo publish -p use-time --dry-run
```
